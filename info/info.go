...
func GetInfo() (*InfoDisplay, error) {
	display := &InfoDisplay{
		Name:     fmt.Sprintf("        Program: %v", ProgramName),
		Platform: fmt.Sprintf("       Platform: %v %v", runtime.GOOS, runtime.GOARCH),
		Runtime:  fmt.Sprintf("        Runtime: %v", runtime.Version()),
	}
	hostname, err := os.Hostname()
	if err != nil {
		hostname = fmt.Sprintf("Fail to get hostname: %v", err)
	}
	display.HostName = fmt.Sprintf("       Hostname: %v", hostname)
	...
	display.BuildCommit = fmt.Sprintf("   Build Commit: %v", buildCommit)
